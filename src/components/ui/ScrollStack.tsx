...
export const ScrollStack = ({
  ...
  const measureOffsets = useCallback(() => {
    ...
    // 1. Temporarily clear transforms and filters to measure clean, untranslated values
    const originalTransforms = cards.map((c) => (c ? c.style.transform : ""));
    const originalFilters = cards.map((c) => (c ? c.style.filter : ""));
    ...
    // 2. Measure untranslated offsets
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const endElementTop = endElementOffsetRef.current || (endElement ? getElementOffset(endElement) : 0);
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      if (blurAmount) {
        ...
        for (let j = 0; j < cardsRef.current.length; j++) {
          ...
          if (!cardJ) continue;
          const jCardTop = cardOffsetsRef.current[j] ?? getElementOffset(cardJ);
